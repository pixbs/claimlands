...
//! covers every simulation-visible field ([`civ_core::hash::world_hash`]), a
...
//! # Adding one
//!
//! Write the scenario, run `cargo run -p civ-cli -- golden record <file>`, and
...
impl GoldenReplay {
    ...
    pub fn verify(&self) -> Result<(), GoldenError> {
        ...
        let violations = civ_core::invariants::check(session.world());
    ...
    /// `civ-cli golden record`; never call this from a test.
