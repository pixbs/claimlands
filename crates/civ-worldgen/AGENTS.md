# civ-worldgen — local rules

This crate builds the planet: the geodesic sphere, its dual tile graph, and
later the terrain and cover on it. `civ-core` plays the game on whatever comes
...
- **The layering rule applies from the first line of code.** This crate may
  ...
  build otherwise. Nothing above `civ-core` may be reached from here.
- **Floating point is allowed here and only here on this path.** The sphere is
  ...
  `civ-core`, which does no floating-point arithmetic at all.
