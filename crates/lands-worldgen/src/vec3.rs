...
//! `lands-core` is one more thing a reviewer has to audit before trusting a
...
//! once, off any hot path, and `lands-procgen` narrows to `f32` when it fills a
//! vertex buffer.
//!
//! Nothing in this module is used by `lands-core`, which does no floating-point
