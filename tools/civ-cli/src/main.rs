...
//!
//! ```text
//! cargo run -p civ-cli -- fuzz --matches 5000
//! cargo run -p civ-cli -- play --seed 42 --stats
//! cargo run -p civ-cli -- golden verify
//! cargo run -p civ-cli -- golden record tests/replays/capital-split.ron
//! ```
...
use civ_testkit::golden::{GoldenReplay, load_dir};
use civ_testkit::{WorldBuilder, topo};
use clap::{Parser, Subcommand};
use std::path::PathBuf;
use std::process::ExitCode;

#[derive(Parser)]
#[command(name = "civ", about = "Headless tools for the Hex Planet simulation")]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
